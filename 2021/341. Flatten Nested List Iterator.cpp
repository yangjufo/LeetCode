...
public:
    ...
    int next() {
        ...
        int topVal = nestedStack.top().getInteger();
        nestedStack.pop();
        return topVal;
    }
    
    bool hasNext() {
        while (!nestedStack.empty() && !nestedStack.top().isInteger()) {
            vector<NestedInteger> newList = nestedStack.top().getList();
            nestedStack.pop();
            for (int i = (int)newList.size() - 1; i >= 0; i--) {
                nestedStack.push(newList[i]);
            }
        }
