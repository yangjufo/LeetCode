    ...
    void dfs(vector<NestedInteger>& nestedList, int depth, int& ans) {
        ...
        for (int i = 0; i < nestedList.size(); i++) {
            if (nestedList[i].isInteger()) {
                ...
                dfs(nestedList[i].getList(), depth + 1, ans);
            }
        }
...
public:
    int depthSum(vector<NestedInteger>& nestedList) {
        queue<vector<NestedInteger>> nestedQueue;
        queue<int> depthQueue;
        nestedQueue.push(nestedList);
        depthQueue.push(1);
        
        int sum = 0;
        while (!nestedQueue.empty()) {
            vector<NestedInteger>& currList = nestedQueue.front();
            int currDepth = depthQueue.front();
            
            for (auto nested : currList) {
                if (nested.isInteger()) {
                    sum += nested.getInteger() * currDepth;
                } else {
                    nestedQueue.push(nested.getList());
